...
namespace App\Http\Controllers\Web\Backend\V1\CMS\PartnerPage;
...
class PartnerPageHeroBannerController extends Controller
{
    /**
    ...
    public function index()
    {
        $data = CMS::firstOrCreate(
            ['page' => 'partnerPage', 'section' => 'hero'],
            ['title' => null, 'image' => null, 'sub_title' => null]
        );
        return view('backend.layouts.cms.partnerPage.hero', compact('data'));
    }
    ...
    public function store(Request $request)
    {
        ...
        try {
            $data = CMS::where('page', 'partnerPage')->where('section', 'hero')->firstOrFail();
            if ($request->hasFile('image')) {
                ...
                $newImagePath = Helper::fileUpload($request->file('image'), 'partnerPage/HeroBannerImages');
