...
class ReferralSourceCategoryController extends Controller
{
    public function index(Request $request)
    {
        if ($request->ajax()) {
            $data = Category::where('type', 'referralSource')->select(['id', 'name', 'status'])->latest();
            return DataTables::of($data)
                ...
                ->addColumn('action', function ($data) {
                    return '<div class="d-flex gap-2 justify-content-center">
                        <a href="' . route('referral-sources.edit', $data->id) . '" class="btn btn-sm btn-outline-info btn-info-soft" title="Edit">
        ...
        return view('backend.layouts.CategoryFeatures.referralSource.index');
    }
    ...
    {
        return view("backend.layouts.CategoryFeatures.referralSource.create");
    }
    ...
    {
        $data = Category::where('type', 'referralSource')->findOrFail($id);
        return view("backend.layouts.CategoryFeatures.referralSource.edit", compact("data"));
    }
    ...
    public function update(Request $request, string $id)
    {
        ...
        try {
            ...
            return redirect()->route('referral-sources.index')->with('t-success', 'Data updated successfully!');
        } catch (Exception $e) {
    ...
    {
        $data = Category::where('type', 'referralSource')->find($id);
        if (! $data) {
            return response()->json([
                "success" => false,
