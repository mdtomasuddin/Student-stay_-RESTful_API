                ...
                <div class="col-lg-11">
                    <div class="card">
                        <div class="card-header d-flex justify-content-between align-items-center">
                            ...
                            <a href="{{ route('course.create') }}" class="btn btn-primary btn-sm" id="addNewPage">Add Course
...
@push('scripts')
    <script>
        ...
        async function showCourseDetails(id) {
            ...
            let url = '{{ route('course.show', ['course' => ':id']) }}';
                ...
                if (response.data && response.data.data) {
                    ...
                    modalBody.innerHTML = `
                        <div class="text-center mb-3">
                            <a href="${data.thumbnail ? '/' + data.thumbnail : '#'}" target="_blank">
                                <img src="${data.thumbnail ? '/' + data.thumbnail : '/backend/images/no-image.png'}" alt="" style="width: 200px; height: 150px; object-fit: cover; border-radius: 8px; border: 1px solid #ddd;">
                            </a>
                        </div>
                        <div class="mb-3">
                            <strong>Title:</strong> 
        ...
        function statusChange(id) {
            let url = '{{ route('course.status', ['id' => ':id']) }}'.replace(':id', id);
            $.ajax({
                type: "GET",
                url: url,
        ...
        function deleteItem(id) {
            let url = '{{ route('course.destroy', ['course' => ':id']) }}'.replace(':id', id);
            $.ajax({
                type: "DELETE",
                url: url,
                success: function(resp) {
