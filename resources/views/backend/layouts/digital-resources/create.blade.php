...
@section('title')
    {{ env('APP_NAME') }} || Create Digital Resource
@endsection
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div id="edit" class="mb-4">
                                    <h2 class="h3 mb-1">Create Digital Resource</h2>
                                    <p>Create the details below and submit.</p>
                                </div>

                                <div class="card mb-10">
                                    <div class="tab-content p-4">
                                        <form action="{{ route('digital-resources.store') }}" method="POST"
                                            ...
                                            {{-- <div class="form-group mb-4">
                                                ...
                                                <select name="access"
                                                    ...
                                                    <option value="free" {{ old('access') === 'free' ? 'selected' : '' }}>
                                                        Free</option>
                                                    <option value="paid" {{ old('access') === 'paid' ? 'selected' : '' }}>
                                                        Paid</option>
                                                </select>
                                                @error('access')
                                                    <div class="text-danger">{{ $message }}</div>
                                            ...
                                            <div class="form-group mb-4">
                                                ...
                                                <textarea class="form-control text-dark ps-3 @error('description') is-invalid @enderror" name="description"
                                                    placeholder="Enter description here" rows="4">{{ old('description') }}</textarea>
                                                @error('description')
                                                    <div class="text-danger">{{ $message }}</div>
                                                @enderror
                                            </div>
